...
Each physical server can be managed only by VNFMs on an admissible subset of
servers. Equation~\eqref{eq:mgmt-relationship} enforces this: if VNF \(v\) of
chain \(r\) is placed on server \(i\) and the chain's VNFM is placed on server
\(j\), then \(j\) must be allowed to manage \(i\) (\(\eta_{i,j} = 1\)).

\begin{equation}\label{eq:mgmt-relationship}
\begin{aligned}
& z^t_{vi} + \bar{z}_{rj} \le 1 + \eta_{i,j}, \\
& \forall i, j \in V^p,\ \forall r \in R,\ \forall v \in V^s_r,\ \forall t \in T
\end{aligned}
\end{equation}

When \(\eta_{i,j} = 1\) the right-hand side is \(2\) and the constraint is
inactive; when \(\eta_{i,j} = 0\) it reduces to \(z^t_{vi} + \bar{z}_{rj} \le 1\),
forbidding the co-occurrence of a VNF on \(i\) with its manager on \(j\).
...
The problem is an ILP and contains bin packing as a special case, so it is
NP-hard.
