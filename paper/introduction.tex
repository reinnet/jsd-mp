...
\cite{Bari2015} and \cite{AbuLebdeh2017} that improves both implementation time
and final output.
...
Placement Problem and the Manager Placement Problem \emph{jointly} and places
chains and their corresponding VNFMs at the same time. We believe this work has
not been done in the literature before. By considering the joint problem, the
placement avoids committing VNFs to a layout that no admissible manager can
serve, so chains a management-blind pipeline would strand stay feasible. The
benefit is therefore a \emph{feasibility} effect that is \emph{conditional} on
the management constraints binding, and the results in
Section~\ref{sec:joint-vs-disjoint}, Section~\ref{sec:acceptance}, and
Section~\ref{sec:sweep} confirm it.

The paper is structured as follows. In Section~\ref{sec:related-works} we review
the literature; we then describe the problem in Section~\ref{sec:system-model}
and formulate it in Section~\ref{sec:formulation} as an ILP. In
Section~\ref{sec:solution} we propose a polynomial-time solution and evaluate its
results in Section~\ref{sec:results}. We conclude the paper in
